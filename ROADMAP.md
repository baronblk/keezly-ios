...
| M11.6 `fastlane qa` and `fastlane release_check` | **PARTIAL** — `lint` and `release_check` run and green (7 of 7 automated gates); `tests` green. `ui_tests` is **RED** on a characterised order dependency (ISS-020), so `qa` cannot pass until that is settled |
